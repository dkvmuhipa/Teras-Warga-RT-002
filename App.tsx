...
const PublicServices = ({ pdfConfig }: { pdfConfig: PdfConfig }) => {
  ...
  const handleLetterSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if(!letterForm.applicantName) return;
    await addLetterToDb({ ...letterForm, date: new Date().toISOString().split('T')[0], status: 'Pending' } as LetterRequest);
  ...
  const handleReportSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await addReportToDb({ ...reportForm, date: new Date().toISOString().split('T')[0], status: 'Baru' });
    alert('Laporan berhasil dikirim!');
    navigate('/');
  };

  const draftSurat = () => {
    if(!letterForm.applicantName) { alert('Lengkapi nama pemohon'); return; }
    generateSuratPengantar(letterForm as LetterRequest, pdfConfig, true);
  };

  return (
    <div className="max-w-4xl mx-auto px-4 py-8 mb-20">
      <h1 className="text-3xl font-bold mb-6">Layanan Digital</h1>
      <div className="flex gap-4 mb-6">
        <button onClick={() => setActiveTab('surat')} className={`flex-1 py-3 rounded-xl font-bold transition-all ${activeTab === 'surat' ? 'bg-brand-blue text-white shadow-lg shadow-blue-200' : 'bg-white border text-slate-600'}`}>Buat Surat</button>
        <button onClick={() => setActiveTab('lapor')} className={`flex-1 py-3 rounded-xl font-bold transition-all ${activeTab === 'lapor' ? 'bg-rose-600 text-white shadow-lg shadow-rose-200' : 'bg-white border text-slate-600'}`}>Lapor Masalah</button>
      </div>
      
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
        {activeTab === 'surat' ? (
           <form onSubmit={handleLetterSubmit} className="space-y-4">
              <h3 className="font-bold text-lg border-b pb-2">Formulir Permohonan Surat</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><label className="block text-xs font-bold mb-1">Jenis Surat</label><select className="w-full p-2 border rounded" value={letterForm.type} onChange={e=>setLetterForm({...letterForm, type: e.target.value as any})}><option>Pengantar KTP</option><option>Pengantar KK</option><option>Domisili</option><option>Kematian</option><option>Kelahiran</option><option>Surat Keterangan Usaha (SKU)</option><option>Surat Izin Keramaian</option></select></div>
                  <div><label className="block text-xs font-bold mb-1">Nama Lengkap</label><input className="w-full p-2 border rounded" value={letterForm.applicantName} onChange={e=>setLetterForm({...letterForm, applicantName: e.target.value})} required/></div>
                  <div><label className="block text-xs font-bold mb-1">NIK</label><input className="w-full p-2 border rounded" value={letterForm.nik} onChange={e=>setLetterForm({...letterForm, nik: e.target.value})} required/></div>
                  <div><label className="block text-xs font-bold mb-1">Kepala Keluarga</label><input className="w-full p-2 border rounded" value={letterForm.familyHeadName} onChange={e=>setLetterForm({...letterForm, familyHeadName: e.target.value})}/></div>
                  <div><label className="block text-xs font-bold mb-1">Tempat Lahir</label><input className="w-full p-2 border rounded" value={letterForm.birthPlace} onChange={e=>setLetterForm({...letterForm, birthPlace: e.target.value})}/></div>
                  <div><label className="block text-xs font-bold mb-1">Tanggal Lahir</label><input type="date" className="w-full p-2 border rounded" value={letterForm.birthDate} onChange={e=>setLetterForm({...letterForm, birthDate: e.target.value})}/></div>
                  <div><label className="block text-xs font-bold mb-1">Agama</label><select className="w-full p-2 border rounded" value={letterForm.religion} onChange={e=>setLetterForm({...letterForm, religion: e.target.value})}><option>Islam</option><option>Kristen</option><option>Katolik</option><option>Hindu</option><option>Buddha</option><option>Konghucu</option></select></div>
                  <div><label className="block text-xs font-bold mb-1">Pekerjaan</label><input className="w-full p-2 border rounded" value={letterForm.job} onChange={e=>setLetterForm({...letterForm, job: e.target.value})}/></div>
                  <div><label className="block text-xs font-bold mb-1">Keperluan</label><textarea className="w-full p-2 border rounded" value={letterForm.purposeDetail} onChange={e=>setLetterForm({...letterForm, purposeDetail: e.target.value})}/></div>
              </div>
              <div className="flex gap-2 pt-4">
                  <Button type="button" onClick={draftSurat} variant="outline" className="flex-1">Preview Draft PDF</Button>
                  <Button type="submit" className="flex-1">Kirim Permohonan</Button>
              </div>
           </form>
        ) : (
           <form onSubmit={handleReportSubmit} className="space-y-4">
               <h3 className="font-bold text-lg border-b pb-2 text-rose-600">Formulir Laporan Warga</h3>
               <div><label className="block text-xs font-bold mb-1">Jenis Laporan</label><select className="w-full p-2 border rounded" value={reportForm.type} onChange={e=>setReportForm({...reportForm, type: e.target.value as any})}><option>Keamanan</option><option>Kebersihan</option><option>Fasilitas</option><option>Lainnya</option></select></div>
               <div><label className="block text-xs font-bold mb-1">Isi Laporan</label><textarea className="w-full p-2 border rounded h-32" value={reportForm.description} onChange={e=>setReportForm({...reportForm, description: e.target.value})} required placeholder="Jelaskan masalah secara detail..."/></div>
               <div><label className="block text-xs font-bold mb-1">Lokasi (Blok/Rumah)</label><input className="w-full p-2 border rounded" value={reportForm.houseId} onChange={e=>setReportForm({...reportForm, houseId: e.target.value})} placeholder="Contoh: C5-10"/></div>
               <div><label className="block text-xs font-bold mb-1">Nama Pelapor</label><input className="w-full p-2 border rounded" value={reportForm.reporterName} onChange={e=>setReportForm({...reportForm, reporterName: e.target.value})} required/></div>
               <Button type="submit" variant="danger" className="w-full">Kirim Laporan</Button>
           </form>
        )}
      </div>
    </div>
...
const PublicUMKM = ({ umkm }: { umkm: UMKM[] }) => (
    <div className="max-w-5xl mx-auto px-4 py-8 mb-20">
        <h1 className="text-3xl font-bold mb-6 text-slate-800">UMKM Warga RT 002</h1>
            ...
            {umkm.map(u => (
                <div key={u.id} className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-all">
                    <div className="h-48 bg-slate-200 relative">
                        <img src={u.image} alt={u.name} className="w-full h-full object-cover" onError={(e)=>{(e.target as HTMLImageElement).src='https://via.placeholder.com/300x200?text=No+Image'}} />
                        <span className="absolute top-2 right-2 bg-white/90 px-2 py-1 rounded-lg text-xs font-bold shadow-sm">{u.category}</span>
                    </div>
                    <div className="p-4">
                        <h3 className="font-bold text-lg text-slate-800">{u.name}</h3>
                        <p className="text-xs text-slate-500 mb-2">Pemilik: {u.owner}</p>
                        <p className="text-sm text-slate-600 line-clamp-2 mb-4">{u.description}</p>
                        <a href={`https://wa.me/${u.contact}`} target="_blank" rel="noreferrer" className="block w-full text-center bg-brand-blue text-white py-2 rounded-lg font-bold text-sm hover:bg-sky-600 transition-colors">
                            Hubungi via WhatsApp
                        </a>
                    </div>
                </div>
            ))}
            {umkm.length === 0 && <p className="text-slate-500 col-span-full text-center py-10">Belum ada data UMKM.</p>}
        </div>
    </div>
);
...
const PublicInfo = ({ announcements, ronda, officials, cashFlow }: { announcements: Announcement[], ronda: RondaSchedule[], officials: Official[], cashFlow: CashFlow[] }) => (
    <div className="max-w-5xl mx-auto px-4 py-8 mb-20 space-y-8">
        <div>
            <h2 className="text-2xl font-bold mb-4 text-slate-800">Struktur Pengurus</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {officials.map(o => (
                    <div key={o.id} className="flex items-center gap-4 p-4 bg-white rounded-xl border border-slate-100 shadow-sm">
                        <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 overflow-hidden">
                             {o.photo ? <img src={o.photo} className="w-full h-full object-cover"/> : <User size={24}/>}
                        </div>
                        <div>
                            <p className="text-xs text-brand-blue font-bold uppercase">{o.role}</p>
        ...
        <div>
            <h2 className="text-2xl font-bold mb-4 text-slate-800">Jadwal Ronda</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {ronda.map(r => (
                    <div key={r.day} className="bg-white p-4 rounded-xl border border-slate-100">
                        <h4 className="font-bold text-brand-blue mb-2">{r.day}</h4>
                        <ul className="text-sm text-slate-600 space-y-1">
                            {r.members.map((m, i) => <li key={i}>• {m}</li>)}
                        </ul>
                    </div>
                ))}
            </div>
        ...
        <div>
            <h2 className="text-2xl font-bold mb-4 text-slate-800">Laporan Kas</h2>
             <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
                <div className="grid grid-cols-2 gap-4 mb-4">
                   <div className="bg-emerald-50 p-4 rounded-xl">
                      <p className="text-xs font-bold text-emerald-600 uppercase">Pemasukan</p>
                      <h3 className="text-xl font-black text-slate-800">Rp {cashFlow.filter(c=>c.type==='Income').reduce((a,b)=>a+b.amount,0).toLocaleString()}</h3>
                   </div>
                   <div className="bg-rose-50 p-4 rounded-xl">
                      <p className="text-xs font-bold text-rose-600 uppercase">Pengeluaran</p>
                      <h3 className="text-xl font-black text-slate-800">Rp {cashFlow.filter(c=>c.type==='Expense').reduce((a,b)=>a+b.amount,0).toLocaleString()}</h3>
                   </div>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-slate-50 text-slate-500 font-bold uppercase text-xs"><tr><th className="p-3">Tanggal</th><th className="p-3">Keterangan</th><th className="p-3 text-right">Nominal</th></tr></thead>
                        <tbody>
                            {cashFlow.slice(0, 5).map(c => (
                                <tr key={c.id} className="border-b last:border-0">
                                    <td className="p-3">{c.date}</td>
                                    <td className="p-3">{c.description}</td>
                                    <td className={`p-3 text-right font-bold ${c.type==='Income'?'text-emerald-600':'text-rose-600'}`}>{c.type==='Income'?'+':'-'} {c.amount.toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex relative">
      ...
      <div className="flex-1 md:ml-64 p-4 md:p-8 pb-safe-area-pb md:pb-8 max-w-full overflow-hidden">
          ...
          {activeTab === 'residents' && (
              <div className="animate-fade-in space-y-6">
                  ...
                  {residentView === 'grid' ? (<div className="overflow-x-auto rounded-3xl border border-slate-200 shadow-sm"><HouseMap houses={houses} isAdmin={true} onEditHouse={openEditHouse} onPayDues={openDuesModal} reports={reports} officials={officials} /></div>) : (
                      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden overflow-x-auto"><table className="w-full text-sm text-left whitespace-nowrap"><thead className="bg-slate-50/50 text-slate-500 font-bold uppercase text-[10px] tracking-wider border-b border-slate-100"><tr><th className="px-6 py-4">Kavling Rumah</th><th className="px-6 py-4">Kepala Keluarga / P.J.</th><th className="px-6 py-4">Status & Kontak</th><th className="px-6 py-4">Kewajiban Iuran</th><th className="px-6 py-4 text-center">Aksi</th></tr></thead><tbody className="divide-y divide-slate-50">{houses.filter((h:House) => h.headOfFamily.toLowerCase().includes(searchResident.toLowerCase()) || h.id.toLowerCase().includes(searchResident.toLowerCase())).map(h => (<tr key={h.id} className="hover:bg-slate-50"><td className="px-6 py-4">{h.block}-{h.number}</td><td className="px-6 py-4">{h.headOfFamily}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded text-[10px] font-bold ${h.status === 'Occupied' ? (h.residenceType === 'Kost' ? 'bg-cyan-50 text-cyan-600' : 'bg-blue-50 text-blue-600') : h.status === 'Empty' ? 'bg-slate-100 text-slate-500' : 'bg-purple-50 text-purple-600'}`}>{h.status === 'Occupied' ? (h.residenceType === 'Kost' ? 'Kost/Mhs' : (h.residenceType === 'Kontrak' ? 'Dihuni (Sewa)' : 'Dihuni (Tetap)')) : h.status === 'Empty' ? 'Kosong' : 'Usaha'}</span></td><td className="px-6 py-4">{h.paymentStatus}</td><td className="px-6 py-4 text-center"><button onClick={() => openEditHouse(h)} className="p-2 text-slate-400 hover:text-brand-blue"><Edit2 size={16} /></button></td></tr>))}</tbody></table></div>
                  )}
              </div>
          )}
          ...
          {isModalOpen && (
             <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalType === 'announcement' ? "Buat Pengumuman" : modalType === 'cash' ? "Catat Transaksi" : modalType === 'official' ? "Data Pengurus" : modalType === 'inventory' ? "Data Inventaris" : modalType === 'ronda' ? "Jadwal Ronda" : modalType === 'umkm' ? "Data UMKM" : modalType === 'dues' ? "Catat Iuran" : modalType === 'import' ? "Verifikasi Import CSV" : "Edit Data Warga"}>
                 ...
                 {modalType === 'editHouse' && (
                     <form onSubmit={handleSaveHouse} className="space-y-4">
                        <div><label className="block text-xs font-bold mb-1">{editHouseForm.residenceType === 'Kost' ? 'Penanggung Jawab / Perwakilan' : 'Kepala Keluarga'}</label><input className="w-full p-2 border rounded" value={editHouseForm.headOfFamily} onChange={e=>setEditHouseForm({...editHouseForm, headOfFamily: e.target.value})}/></div>
                     ...
                     <div className="space-y-4">
                         <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex gap-3">
                             ...
                             <div className="text-sm text-blue-800">
                                 <p className="font-bold">Konfirmasi Import Data</p>
                                 <p>Sistem menemukan <strong>{importPreview.length}</strong> baris data siap import.</p>
                                 <p className="mt-1 text-xs">Periksa tabel di bawah sebelum melanjutkan. Data dengan Nomor Blok yang sama akan <strong>ditimpa (update)</strong>.</p>
                             </div>
                         </div>
                         
                         <div className="max-h-60 overflow-y-auto border rounded-lg custom-scrollbar">
                             <table className="w-full text-xs text-left">
                                 <thead className="bg-slate-100 sticky top-0">
                                     <tr>
                                         <th className="p-2 border-b">No</th>
                                         <th className="p-2 border-b">Blok</th>
                                         <th className="p-2 border-b">Kepala Keluarga</th>
                                         <th className="p-2 border-b">Status</th>
                                         <th className="p-2 border-b">Iuran</th>
                                     </tr>
                                 </thead>
                                 <tbody>
                                     {importPreview.map((row, idx) => (
                                         <tr key={idx} className="border-b last:border-0 hover:bg-slate-50">
                                             ...
                                             <td className="p-2 font-bold">{row.block}-{row.number}</td>
                                             <td className="p-2">{row.headOfFamily}</td>
                                             <td className="p-2">{row.status === 'Occupied' ? 'Dihuni' : row.status}</td>
                                             <td className={`p-2 font-bold ${row.paymentStatus === PaymentStatus.PAID ? 'text-green-600' : 'text-red-600'}`}>{row.paymentStatus}</td>
                                         </tr>
                                     ))}
                                 </tbody>
                             </table>
                         ...
                         <div className="flex gap-3 pt-2">
                             ...
                             <Button onClick={executeImport} className="flex-1" disabled={isImporting}>
                                 {isImporting ? <Loader2 className="animate-spin" /> : <CheckCircle size={18} />}
                                 {isImporting ? 'Memproses...' : 'Ya, Proses Import'}
                             </Button>
                         </div>
                     </div>
                 )}
...
export const App = () => {
  ...
  useEffect(() => {
    ...
    return () => {
        unsubHouses(); unsubAnn(); unsubCash(); unsubOff(); unsubRep(); 
        unsubLet(); unsubRonda(); unsubInv(); unsubUmkm();
    };
  }, []);

  if (loading) return <div className="h-screen flex items-center justify-center"><Loader2 className="animate-spin text-brand-blue" size={48}/></div>;
            ...
            <Route path="*" element={
                <div className="min-h-screen bg-slate-50 relative pb-20 md:pb-0">
                    ...
                    <Routes>
                        ...
                        <Route path="/umkm" element={<PublicUMKM umkm={umkm} />} />
                        <Route path="/info" element={<PublicInfo announcements={announcements} ronda={ronda} officials={officials} cashFlow={cashFlow} />} />
                    </Routes>
