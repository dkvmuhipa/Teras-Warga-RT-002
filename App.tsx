             ...
             <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalType === 'announcement' ? "Buat Pengumuman" : modalType === 'poll' ? "Buat Voting Baru" : modalType === 'cash' ? (editingCashId ? "Edit Transaksi" : "Catat Transaksi") : modalType === 'official' ? "Data Pengurus" : modalType === 'inventory' ? "Data Inventaris" : modalType === 'ronda' ? "Jadwal Ronda" : modalType === 'umkm' ? "Kelola Data UMKM" : modalType === 'dues' ? "Catat Iuran" : modalType === 'bulkDues' ? "Update Iuran Massal" : modalType === 'import' ? "Import Data Warga" : "Edit Data Warga"}>
                 ...
                 {modalType === 'editHouse' && (
                     <form onSubmit={handleSaveHouse} className="space-y-4">
                         ...
                         <div className="grid grid-cols-2 gap-3">
                             <div><label className="block text-xs font-bold mb-1.5">Jumlah Penghuni</label><input type="number" className="w-full p-3 border rounded-xl" value={editHouseForm.occupants} onChange={e=>setEditHouseForm({...editHouseForm, occupants: parseInt(e.target.value)})}/></div>
                         ...
                         <div className="grid grid-cols-2 gap-3">
                             <div>
                                 <label className="block text-xs font-bold mb-1.5">Status Hunian</label>
                                 <select className="w-full p-3 border rounded-xl" value={editHouseForm.unifiedStatus} onChange={e=>setEditHouseForm({...editHouseForm, unifiedStatus: e.target.value})}>
                             ...
                             <div>
                                 <label className="block text-xs font-bold mb-1.5">Status Iuran</label>
                                 <select className="w-full p-3 border rounded-xl" value={editHouseForm.paymentStatus} onChange={e=>setEditHouseForm({...editHouseForm, paymentStatus: e.target.value})}>
                         ...
                         {/* Demographics */}
                         <div>
                             <label className="block text-xs font-bold mb-2">Data Demografi Khusus</label>
                             <div className="grid grid-cols-2 gap-2">
                                 {DEMOGRAPHIC_OPTIONS.map((opt) => (
                                     <label key={opt.key} className="flex items-center gap-2 p-2 border rounded-lg cursor-pointer hover:bg-slate-50">
                                         <input 
                                             type="checkbox" 
                                             checked={!!(editHouseForm as any)[opt.key]} 
                                             onChange={e => setEditHouseForm({...editHouseForm, [opt.key]: e.target.checked})}
