...
import { LetterRequest, PdfConfig, House, PaymentStatus, CashFlow } from "../types";
...
export const generateFinancialReportPDF = async (transactions: CashFlow[], customConfig?: PdfConfig) => {
    const config = customConfig || DEFAULT_PDF_CONFIG;
    ...
    doc.setFontSize(14);
    doc.text("PEMERINTAH KOTA PALU", centerX, 14, { align: "center" });
    doc.text("KELURAHAN TONDO - KECAMATAN MANTIKULORE", centerX, 20, { align: "center" });
    doc.text("PENGURUS RT.002/RW.020", centerX, 26, { align: "center" });
    doc.setFontSize(10);
    doc.text(`Alamat : ${config.rtAddress}`, centerX, 32, { align: "center" });
    ...
    doc.setFont("times", "bold");
    doc.setFontSize(12);
    doc.text("LAPORAN ARUS KAS (CASHFLOW)", centerX, 45, { align: "center" });
    doc.setFontSize(10);
    doc.text(`Periode: Hingga ${dateString}`, centerX, 50, { align: "center" });

    // --- TABLE CONFIG ---
    let startY = 60;
    const rowHeight = 8;
    // Columns: No, Tanggal, Kategori, Deskripsi, Masuk (Debit), Keluar (Kredit)
    const cols = [
        { header: "No", width: 15, x: margin },
        { header: "Tanggal", width: 35, x: margin + 15 },
        { header: "Kategori", width: 40, x: margin + 50 },
        { header: "Uraian / Deskripsi", width: 90, x: margin + 90 },
        { header: "Pemasukan (Rp)", width: 45, x: margin + 180 },
        { header: "Pengeluaran (Rp)", width: 45, x: margin + 225 },
    ];
    ...
    const drawTableHeader = (y: number) => {
        ...
        cols.forEach(col => {
            if(col.header.includes("(Rp)")) doc.text(col.header, col.x + col.width - 2, y + 5, {align: "right"});
            else doc.text(col.header, col.x + 2, y + 5);
        });
    ...
    sortedTx.forEach((tx, index) => {
        ...
        if(tx.type === 'Income') totalIncome += amount; else totalExpense += amount;

        const incomeStr = tx.type === 'Income' ? amount.toLocaleString('id-ID') : "-";
        const expenseStr = tx.type === 'Expense' ? amount.toLocaleString('id-ID') : "-";

        doc.text((index + 1).toString(), cols[0].x + 2, currentY + 5);
        doc.text(new Date(tx.date).toLocaleDateString('id-ID'), cols[1].x + 2, currentY + 5);
        doc.text(tx.category, cols[2].x + 2, currentY + 5);
        const splitDesc = doc.splitTextToSize(tx.description, cols[3].width - 4);
        doc.text(splitDesc, cols[3].x + 2, currentY + 5);
        doc.text(incomeStr, cols[4].x + cols[4].width - 2, currentY + 5, {align: "right"});
        doc.text(expenseStr, cols[5].x + cols[5].width - 2, currentY + 5, {align: "right"});
        ...
        currentY += rowHeight;
        if(splitDesc.length > 1) currentY += (splitDesc.length - 1) * 4; // Adjust overlap if text wrapped
    });
    ...
    doc.text("TOTAL PEMASUKAN", margin + 150, currentY, {align: "right"});
    doc.text(`Rp ${totalIncome.toLocaleString('id-ID')}`, pageWidth - margin - 5, currentY, {align: "right"});
    currentY += 6;
    doc.text("TOTAL PENGELUARAN", margin + 150, currentY, {align: "right"});
    doc.text(`Rp ${totalExpense.toLocaleString('id-ID')}`, pageWidth - margin - 5, currentY, {align: "right"});
    currentY += 2;
    doc.setLineWidth(0.5);
    doc.line(margin + 120, currentY, pageWidth - margin, currentY);
    currentY += 6;
    doc.setFontSize(12);
    doc.text("SALDO AKHIR", margin + 150, currentY, {align: "right"});
    doc.text(`Rp ${balance.toLocaleString('id-ID')}`, pageWidth - margin - 5, currentY, {align: "right"});

    // --- SIGNATURE ---
    const signY = currentY + 15;
    if (signY + 40 > pageHeight) doc.addPage();
    const signX = pageWidth - 60;

    doc.setFontSize(10);
    doc.text(`Palu, ${dateString}`, signX, signY, { align: "center" });
    doc.text("Bendahara RT 002", signX, signY + 6, { align: "center" });

    doc.setFont("times", "bold", "underline");
    doc.text("Rudi Hartono", signX, signY + 35, { align: "center" }); // Mock Bendahara name or fetch from officials

    doc.save(`Laporan_Keuangan_RT002_${new Date().toISOString().split('T')[0]}.pdf`);
};
