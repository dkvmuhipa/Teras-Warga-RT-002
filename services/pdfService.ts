...
export const generateResidentReportPDF = async (houses: House[], customConfig?: PdfConfig) => {
    ...
    sortedHouses.forEach((house, index) => {
        ...
        const status = house.status === 'Occupied' ? 'Dihuni' : (house.status === 'Business' ? 'Usaha' : 'Kosong');
        ...
        doc.text(status, cols[4].x + 1, currentY + 5);
    ...
    currentY += 5;
    doc.text(`Rumah Dihuni/Usaha: ${totalOccupied}`, margin, currentY);
    doc.text(`Rumah Kosong: ${totalEmpty}`, margin + 70, currentY);
    currentY += 5;
