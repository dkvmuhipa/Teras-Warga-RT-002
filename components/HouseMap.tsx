...
import { CheckCircle, Home, MapPin, Store, MinusCircle, User, Info, ChevronDown, ChevronUp, Filter } from 'lucide-react';
...
export const HouseMap: React.FC<HouseMapProps> = ({ houses, isAdmin, onHouseClick }) => {
  ...
  const getStatusStyles = (house: House) => {
  ...
  const getStatusIcon = (house: House) => {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl shadow-slate-200 border border-slate-100 overflow-hidden animate-slide-up transition-all duration-300">
      ...
      <div className={`transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-[450px]' : 'max-h-0'} bg-slate-50/50 relative group`}>
        <div className="overflow-y-auto max-h-[450px] p-5 custom-scrollbar">
              ...
              return (
                <div key={block} className="mb-8 last:mb-0 animate-fade-in">
                  <div className="flex items-center gap-3 mb-3 sticky top-0 z-10 py-2 -mx-2 px-2 bg-slate-50/95 backdrop-blur-sm border-b border-slate-200/50">
                    <span className="text-xs font-black text-slate-500 bg-white px-2 py-1 rounded border border-slate-200 shadow-sm">
                      BLOK {block}
                    </span>
                    ...
                    {blockHouses.map((house) => (
                      <button
                        ...
                        className={`
                          ...
                          hover:-translate-y-0.5 hover:z-10 w-full aspect-square
                          ${getStatusStyles(house)}
                        `}
                      >
                        ...
                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/house:block w-40 z-30 pointer-events-none">
                            <div className="bg-slate-800 text-white p-2.5 rounded-lg shadow-xl border border-slate-700 text-left relative">
                                <div className="flex items-center gap-2 mb-2 border-b border-slate-600 pb-1.5">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-slate-400">{house.status === 'Empty' ? 'Kosong' : 'Dihuni'}</p>
                                    </div>
                                </div>
        ...
        {/* Scroll Indicator Gradient - Triggers on Parent Group hover */}
      ...
      {/* Legend Footer */}
      {isExpanded && (
        <div className="bg-white border-t border-slate-100 p-3 animate-fade-in">
