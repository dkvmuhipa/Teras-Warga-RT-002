      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Edit Berita" : "Buat Berita Baru"}>
        <form onSubmit={handleSaveNews} className="space-y-5">
          ...
          <div>
            ...
            <textarea className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all min-h-[120px]" rows={4} value={content} onChange={e=>setContent(e.target.value)} placeholder="Tulis isi berita di sini..." required/>
          </div>
