...
import { FileText, Plus, Search, Filter, Edit2, Trash2, Printer, Send, Eye, X, Save, Calendar, User, MessageSquare, Upload, Download, File as FileIcon } from 'lucide-react';
...
export const OfficialLetterManager: React.FC<OfficialLetterManagerProps> = ({ pdfConfig, setPdfConfig }) => {
  ...
  useEffect(() => {
    if (isModalOpen && !editingLetter) {
      // Auto-generate letter number if creating new
  ...
  }, [isModalOpen, editingLetter, formData.type, pdfConfig.lastLetterNumber, pdfConfig.rtName]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (formData.source === 'External' && !selectedFile && !formData.attachmentUrl) {
    ...
    try {
      ...
      if (selectedFile) {
        const path = `official-letters/${Date.now()}_${selectedFile.name}`;
  ...
  const handlePrint = async (letter: OfficialLetter) => {
    if (letter.source === 'External' && letter.attachmentUrl) {
      window.open(letter.attachmentUrl, '_blank');
      return;
    }
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-2">
                    ...
                    {letter.source === 'External' && (
                      <div className="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-amber-100 text-amber-600 flex items-center gap-1">
                        <Upload size={10} /> File Luar
                      </div>
                    )}
                  </div>
                  <div className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="flex p-1 bg-slate-100 rounded-2xl border border-slate-200">
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                formData.source === 'Internal' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Buat di Aplikasi
            </button>
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                formData.source === 'External' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
              }`}
            >
              Upload File Luar
            </button>
          ...
          <div className="space-y-2">
            <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">
              {formData.source === 'External' ? 'Lampiran Surat (Wajib)' : 'Lampiran Surat (Opsional)'}
            </label>
            <div className={`relative border-2 border-dashed rounded-2xl p-6 transition-all ${
              selectedFile ? 'border-indigo-500 bg-indigo-50/30' : 
              (formData.source === 'External' && !formData.attachmentUrl) ? 'border-amber-300 bg-amber-50/20' : 'border-slate-200 bg-slate-50 hover:border-indigo-300'
            }`}>
              <input 
                type="file" 
                ...
                accept=".pdf,image/*"
                required={formData.source === 'External' && !formData.attachmentUrl}
              />
              <div className="flex flex-col items-center justify-center gap-2">
                {selectedFile ? (
                  ...
                  <>
                    <Upload className={formData.source === 'External' ? "text-amber-400" : "text-slate-400"} size={32} />
