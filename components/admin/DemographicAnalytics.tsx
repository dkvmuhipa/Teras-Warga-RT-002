...
import { 
  ...
  ChevronRight, Info, Sparkles, FileText
} from 'lucide-react';
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black text-indigo-600 uppercase tracking-[0.3em]">Demographic Intelligence</span>
          </div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tight">Analitik Kependudukan</h2>
          <p className="text-slate-500 font-medium mt-2 max-w-2xl">
            Visualisasi data kependudukan RT 002 yang komprehensif untuk mendukung pengambilan keputusan berbasis data dan perencanaan sosial yang lebih baik.
          </p>
        </div>
        <div className="flex gap-2">
          <div className="px-4 py-2 bg-white border border-slate-200 rounded-2xl shadow-sm flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
            <span className="text-xs font-bold text-slate-600">Data Real-time</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          >
            <div className={`absolute top-0 right-0 w-24 h-24 bg-${stat.color}-500/5 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-700`}></div>
            <div className="flex items-center gap-5 relative z-10">
              <div className={`p-4 bg-${stat.color}-50 text-${stat.color}-600 rounded-2xl group-hover:rotate-6 transition-transform`}>
                <stat.icon size={24} />
              </div>
              <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <motion.div variants={itemVariants} className="bg-gradient-to-br from-indigo-600 to-violet-700 p-8 rounded-[3rem] shadow-xl shadow-indigo-600/20 text-white relative overflow-hidden">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
        ...
        <motion.div variants={itemVariants} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm">
        ...
        <motion.div variants={itemVariants} className="lg:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col items-center text-center group hover:bg-slate-50 transition-all">
              <div className={`p-4 bg-${item.color}-50 text-${item.color}-600 rounded-2xl mb-4 group-hover:scale-110 transition-transform`}>
                <item.icon size={20} />
              </div>
      ...
      <motion.div 
        variants={itemVariants}
        className="bg-slate-900 p-10 rounded-[3.5rem] text-white relative overflow-hidden"
      >
